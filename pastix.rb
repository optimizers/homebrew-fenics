...
class Pastix < Formula
  ...
  url 'https://gforge.inria.fr/frs/download.php/32932/pastix_release_4475.tar.bz2'
  ...
  def install
    ...
    scotch = build.with? 'scotch' ? 'scotch' : 'scotch5'
  ...
  def test
    Dir.foreach("#{share}/example/bin") do |example|
      ...
      next if example == 'reentrant'  # Crashes. Notified upstream.
